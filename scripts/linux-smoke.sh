...
cargo run -- start --run "$run_config"
wait_for_desktop
cargo run -- smoke --run "$run_config" --url "$url" --screenshot "$run_dir/smoke.png"
wait_for_page "$url"
cargo run -- capture --run "$run_config" --output "$run_dir/smoke.png"
cargo run -- drag-proof --run "$run_config" --from-x 400 --from-y 350 --to-x 800 --to-y 350
cargo run -- reset --run "$run_config"
wait_for_desktop
cargo run -- capture --run "$run_config" --output "$run_dir/reset.png"
